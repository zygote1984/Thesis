...
The step is to find the the suitable conversion for the given input by calling the \lstinln{findConversion} method (line~\ref{findConversion}).
...
The reflective method \lstinln{getInterfaces} is used to return the interface a type implements, which is called on line~\ref{lst:itc:e}. These interface are added to the list \lstinln{sourceTypeHier} by iterating over the list \lstinln{itc}.
...
In a \lstinln{try-catch} statement, we first create a \lstinln{Class} object for the converter that contains the \lstinln{conversion} (line~\ref{lst:getconvclass}). 
