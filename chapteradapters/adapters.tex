...
\TODO{Assumptions should be explicit in the beginning}
...
...
%; this operation is called data binding.
...
\TODO{make it better}The adapter pattern makes two interfaces compatible by adapting the instances of a source interface to work with a target interface. 
...
\TODO{Limitation, we don't always have control over source code}
Then the concrete implementation of the interface is repeated in the sub-class. 

\TODO{There are two problem statements, better structuring separate into two paragraphs}
The implementation of the object adapter pattern is more flexible, since it only needs to inherit from the target type. 
An issue is that the adapter pattern adds a level of indirection between the source and the target, which needs to be maintained \TODO{Maintained why?}. 
...
\TODO{It should be explicit that the user wants to reuse the adapter}
...
\TODO{How would you know if it is an adapter if it is not indicated with naming conventions?}
...
\begin{enumerate}[{R}-1]
\item Inheritance-free adapter structures. (P-1) \TODO{I don't have a restriction, I do not require it!} 
...
%Changing requirements force software systems to include new components to extend their functionality. 
...
\TODO{Rephrase according to the change in introduction}
...
The user-defined input is a produced during \zamkph{development time}. In the figure we show four inputs that are fed to the \zamk compile-time; they are (in the order shown in the figure):
